...
const FORMAT_CONFIGS: Record<VideoFormat, FormatConfig> = {
  ...
  webm: {
    ...
    options: ['-quality', 'realtime', '-cpu-used', '5', '-crf', '10', '-b:v', '1M'],
  },
  avi: {
    extension: 'avi',
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      if (config.audioCodec) {
        ffmpegArgs.push('-c:a', config.audioCodec);
      }
      ...
      toast({
        title: 'Success',
        description: `Video converted to ${format.toUpperCase()} successfully! ${formatFileSize(originalSize)} → ${formatFileSize(blob.size)}`,
      });
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to convert video';
      setLogs(prev => [...prev, `ERROR: ${errorMsg}`]);
          ...
          <div className="space-y-3">
            ...
            <Select value={format} onValueChange={(v) => setFormat(v as VideoFormat)}>
              ...
              <SelectContent>
                ...
                <SelectItem value="webm">WebM (VP8) - Modern web format</SelectItem>
                <SelectItem value="avi">AVI - Classic format</SelectItem>
                <SelectItem value="mov">MOV - QuickTime format</SelectItem>
                <SelectItem value="mkv">MKV - Matroska container</SelectItem>
              </SelectContent>
            </Select>
          </div>
